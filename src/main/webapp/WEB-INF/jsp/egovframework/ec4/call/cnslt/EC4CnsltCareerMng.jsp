...
<%
 /**
  * @Class Name : EC4CnsltCareerMng.java
  * @Description : EC4CnsltCareerMng List 화면
...
$(document).ready(function(){
    ...
    // 상담원 테이블
    cnsltCareerTable = $('#cnsltCareerTable').DataTable({
        "language": lang_kor,
        "select": {
            "style": "single"
        },
        ...
        "searching": false,
        "bServerSide": true,
        "sAjaxSource": "<c:url value='/ec4/call/cnslt/cnsltCareerMngList.do'/>",
        "fnServerParams": function(aoData){
            aoData.push(
                    {"name": "searchCnsltUserId",   "value": $("#cnsltCareerMngVO input[name=searchCnsltUserId]").val()},
                    {"name": "searchCnsltUserNm",   "value": $("#cnsltCareerMngVO input[name=searchCnsltUserNm]").val()},
                    {"name": "searchShopNm",   "value": $("#cnsltCareerMngVO input[name=searchShopNm]").val()},
                    {"name": "searchBeginDe",   "value": $("#cnsltCareerMngVO input[name=searchBeginDe]").val()},
                    {"name": "searchEndDe",     "value": $("#cnsltCareerMngVO input[name=searchEndDe]").val()},
                    {"name": "searchHpno",   	"value": $("#cnsltCareerMngVO input[name=searchHpno]").val()},
                    {"name": "searchTelno",   	"value": $("#cnsltCareerMngVO input[name=searchTelno]").val()}
            );
        },
        "sServerMethod": "POST",
        "columnDefs": [
        ...
        "columns": [
            ...
            {"data":"workComp", "defaultContent": ""},
            {"data":"workBeginDt", "defaultContent": ""},
            {"data":"resignedDt", "defaultContent": ""},
            {"data":"workPart", "defaultContent": ""}
        ]
    });


    // 검색 input search
    $("#cnsltCareerMngVO input").on("keyup change", function(key){

        if(key.keyCode == 13 || $(this).hasClass("datepicker")){

        	cnsltCareerTable.search(this.value).draw();
        }
    ...
    // 검색 select search
    $("#cnsltCareerMngVO select").on("change", function(key){
    ...
    // 선택
    cnsltCareerTable.on("select", function (e, dt, type, indexes) {
    ...
    $("#btnInsert").click(function(){
        ...
        // 등록 팝업 오픈
        fn_cnsltCareer_reg_popup(params);
    });
    ...
    $("#btnUpdate").click(function(){
        ...
        var rowData = cnsltCareerTable.rows(".selected").data();
        if(rowData[0]){
            params.makrCd = rowData[0].makrCd;
            params.workComp = rowData[0].workComp;
            params.cnsltUserId = rowData[0].cnsltUserId;
        }
        
        // 수정 팝업 오픈
        fn_cnsltCareer_mod_popup(params);
    });
            ...
            if(confirm('<spring:message code="common.delete.msg"/>')){
                ...
                if(!params.cnsltUserId){
                    ...
                    $.ajax({
                        ...
                        "url": "<c:url value='/ec4/call/cnslt/deleteCnsltCareer.do'/>",
                        "data": params,
    ...
    $("#searchCnsltUserNm").keyup(function(){
		if($("#cnsltCareerMngVO input[name=searchCnsltUserNm]").val() == ""){
			$("#cnsltCareerMngVO input[name=searchCnsltUserId]").val("");
		}
	});
...
//등록 팝업 오픈
function fn_cnsltCareer_reg_popup(params){

	$("div.modal-content").load("<c:url value='/ec4/call/cnslt/cnsltCareerRegPopup.do'/>", params, function(response, status, xhr) {
...
//수정 팝업 오픈
function fn_cnsltCareer_mod_popup(params){

	if(params.cnsltUserId){

   $("div.modal-content").load("<c:url value='/ec4/call/cnslt/cnsltCareerModPopup.do'/>", params, function(response, status, xhr) {
    ...
    <section class="content">
		...
		<div class="row">
            <div class="col-md-12">
                ...
                <form:form commandName="cnsltCareerMngVO" name="cnsltCareerMngVO" method="post" cssClass="form-horizontal">
        	...
        	<div class="col-xs-12"> <!-- DIV 가로영역을 나누는 용도 -->
                ...
                <div class="box">
                    ...
                    <div class="box-body no-padding" style="height:750px;">
				          <table id="cnsltCareerTable" class="table table-hover">
				            <thead>
				              <tr>
				                <th>순번</th>
				                <th>제조사</th>
