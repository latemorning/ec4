...
<%
 /**
  * @Class Name : EC4CnsltScheduleMng.java
  * @Description : EC4CnsltScheduleMng List 화면
...
<head>
	...
	<!-- AXmodal, axis -->
	<link rel="stylesheet" type="text/css" href="/css/egovframework/ec4/axisj/bulldog/AXJ.min.css" />
	<link rel="stylesheet" type="text/css" href="/css/egovframework/ec4/axicon/axicon.min.css" />
	<script type="text/javascript" src="/js/egovframework/ec4/axisj/AXJ.all.js"></script>
	<script type="text/javascript" src="/js/egovframework/ec4/axisj/AXInput.js"></script>
    ...
    <!-- fullcalendar -->
	<link type="text/css" href='/resource/adminLTE/bower_components/fullcalendar/dist/fullcalendar.min.css' rel='stylesheet' />
	<link type="text/css" href='/resource/adminLTE/bower_components/fullcalendar/dist/fullcalendar.print.css' rel='stylesheet' media='print' />
	<script src='/resource/adminLTE/bower_components/fullcalendar/dist/fullcalendar.min.js'></script>
	<script src='/resource/adminLTE/bower_components/fullcalendar/dist/locale/ko.js'></script>
...
<style>
  ...
  .axi-json-string3.beforeresverve {
    color:#c6ebc6;
  }
  .axi-json-string3.resverve {
  	color:#00cc00;
  }
  .axi-json-string3.complete {
  	color:00004d;
  }  
  .fc-day-number {
    font-size: 12px;
...
<script type="text/javascript" src="/js/egovframework/ec4/cmm/ec4cnsltschedule.js"></script>
...
$(document).ready(function(){
	...
	if("${TestMap.view}" === "basicWeek") {
		$('#calendar').fullCalendar('changeView', "basicWeek");	
	}
	
	//$(".fc-month-button").prop("disabled", true);
	//$(".fc-basicWeek-button").prop("disabled", true);
	$(".fc-month-button").hide();//월 버튼 숨김
	$(".fc-basicWeek-button").hide();//주 버튼 숨김
    ...
    cnsltLicenseTable = $('#cnsltLicenseTable').DataTable({
        ...
        "searching": false,
        "bServerSide": true,
        "sAjaxSource": "<c:url value='/ec4/call/cnslt/cnsltScheduleMngList.do'/>",
        "fnServerParams": function(aoData){
            aoData.push(
                    {"name": "searchCnsltUserId",   "value": $("#cnsltScheduleMngVO input[name=searchCnsltUserId]").val()},
                    {"name": "searchCnsltUserNm",   "value": $("#cnsltScheduleMngVO input[name=searchCnsltUserNm]").val()},
                    {"name": "searchShopNm",   "value": $("#cnsltScheduleMngVO input[name=searchShopNm]").val()},
                    {"name": "searchBeginDe",   "value": $("#cnsltScheduleMngVO input[name=searchBeginDe]").val()},
                    {"name": "searchEndDe",     "value": $("#cnsltScheduleMngVO input[name=searchEndDe]").val()},
                    {"name": "searchHpno",   	"value": $("#cnsltScheduleMngVO input[name=searchHpno]").val()},
                    {"name": "searchTelno",   	"value": $("#cnsltScheduleMngVO input[name=searchTelno]").val()}
            );
        },
        "sServerMethod": "POST",
        "columnDefs": [
    ...
    // 검색 input search
    $("#cnsltScheduleMngVO input").on("keyup change", function(key){
    ...
    // 검색 select search
    $("#cnsltScheduleMngVO select").on("change", function(key){
    ...
    $("#searchCnsltUserNm").keyup(function(){
		if($("#cnsltScheduleMngVO input[name=searchCnsltUserNm]").val() == ""){
			$("#cnsltScheduleMngVO input[name=searchCnsltUserId]").val("");
		}
	});
    ...
    <section class="content">
		...
		<div class="row">
            <div class="col-md-12">
                ...
                <form:form commandName="cnsltScheduleMngVO" name="cnsltScheduleMngVO" method="post" cssClass="form-horizontal">
      	...
      	<div class="row">
        	<div class="col-xs-12"> <!-- DIV 가로영역을 나누는 용도 -->
                ...
                <div class="box">
                    ...
                    <div class="box-body no-padding" style="height:900px;">


						<form style="margin: 0px" id="frm" name="frm" method="post">
							<div class="calendar-informs">
								<span class="axi axi-json-string3 beforeresverve axi-i-16"></span> 교육 
								<span class="axi axi-json-string3 resverve axi-i-16"></span> 월차
								<span class="axi axi-json-string3 complete axi-i-16"></span> 휴가
							</div>    
