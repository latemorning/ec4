...
<%
  ...
  * @Description : EC4CnsltOutBoundTargetCust List 화면
...
<!-- 아래 STYLE은 멀티 SELECT박스용도이며 공통 CSS에 넣고 사용해봤더니 적용이 안되기에 필요한 페이지에만 기입 적용한다. Egov용 CSS는 수정하지말라고함. --> 
<style>
.dropdownBox{
...
$(document).ready(function(){
    ...
    // 대상고객 테이블
    listTargetCustAsignTable = $('#listTargetCustAsignTable').DataTable({
        "language": lang_kor,
        "select": {
            "style": "single"
        },
        ...
        "searching": false,
        "bServerSide": true,
        "sAjaxSource": "<c:url value='/ec4/call/outbd/searchCnsltOutBoundTargetCustList.do'/>",
        "fnServerParams": function(aoData){
            aoData.push(
            		{"name": "searchCstmrCd",   "value": $("#searchTargetCustVO input[name=searchCstmrCd]").val()},
            		{"name": "searchCstmrNm",   "value": $("#searchTargetCustVO input[name=searchCstmrNm]").val()},
            		{"name": "searchCnsltUserId",   "value": $("#searchTargetCustVO input[name=searchCnsltUserId]").val()},
            		{"name": "searchCnsltUserNm",   "value": $("#searchTargetCustVO input[name=searchCnsltUserNm]").val()},
            		{"name": "scriptKnd",   "value": $("#searchTargetCustVO select[name=scriptKnd] option:selected").val()},
            		{"name": "scriptCd",       "value": $("#searchTargetCustVO select[name=scriptCd] option:selected").val()},
            		{"name": "searchBeginDe",   "value": $("#searchTargetCustVO input[name=searchBeginDe]").val()},
            		{"name": "searchEndDe",     "value": $("#searchTargetCustVO input[name=searchEndDe]").val()},
            		{"name": "searchHpno",       "value": $("#searchTargetCustVO input[name=searchHpno]").val()},
            		{"name": "callState",       "value": $("#searchTargetCustVO select[name=callState] option:selected").val()},
            		{"name": "callAsign",       "value": $("#searchTargetCustVO select[name=callAsign] option:selected").val()},
            		{"name": "searchCnsltTyCn",   "value": $("#searchTargetCustVO input[name=cnsltTyCn]").val()},
            		{"name": "searchCnsltTyCnCode",   "value": $("#searchTargetCustVO input[name=cnsltTyCnCode]").val()}
            );
        },
        "sServerMethod": "POST",
        "columnDefs": [
            {"visible": false, "targets": [1,3,4,5,6,13,16]},
            {"orderable": false, "targets": [0]}
        ],
        "columns": [
            {"data":"rn"},//순번
            {"data":"makrCd", "defaultContent": ""},
            {"data":"callCd", "defaultContent": ""},
            {"data":"callcolectGbCd", "defaultContent": ""},
            {"data":"cstmrCd", "defaultContent": ""},
            {"data":"callCstmrCdSeq", "defaultContent": ""},
            {"data":"asignCnsltUserId", "defaultContent": ""},
            {"data":"callColectGbNm", "defaultContent": ""},
            {"data":"cstmrNm", "defaultContent": ""},
            {"data":"hpno", "defaultContent": ""},
            {"data":"scriptKndNm", "defaultContent": ""},
            {"data":"scriptCdNm", "defaultContent": ""},
            {"data":"asignCnsltUserNm", "defaultContent": ""},
            {"data":"recvNo", "defaultContent": ""},
            {"data":"recvDt", "defaultContent": ""},
            {"data":"accptDt", "defaultContent": ""},
            {"data":"callStateNm", "defaultContent": ""}
        ]
    });
    ...
    // 검색 input search
    $("#searchTargetCustVO input").on("keyup change", function(key){

        if(key.keyCode == 13 || $(this).hasClass("datepicker")){

        	listTargetCustAsignTable.search(this.value).draw();
        }
    ...
    // 검색 select search
    $("#searchTargetCustVO select").on("change", function(key){
    ...
    $("#searchCstmrNm").keyup(function(){
		if($("#searchTargetCustVO input[name=searchCstmrNm]").val() == ""){
			$("#searchTargetCustVO input[name=searchCstmrCd]").val("");
		}
	});
	...
	$(document).on("click","#cnsltTypePop",function() {
        ...
        popupParams.form = document.searchTargetCustVO;
		...
		<!-- ############################# 검색조건 ######################  -->
      	<div class="row">
            <div class="col-md-12">
                ...
                <form:form commandName="searchTargetCustVO" name="searchTargetCustVO" method="post" cssClass="form-horizontal">
        	...
        	<div class="col-xs-12"> <!-- DIV 가로영역을 나누는 용도 -->
                ...
                <div class="box">
                    ...
                    <div class="box-body no-padding" style="height:750px;">
							<table id="listTargetCustAsignTable" class="table table-hover">
				            <thead>
				              <tr>
				                <th>순번</th>
				                <th>브랜드</th>
