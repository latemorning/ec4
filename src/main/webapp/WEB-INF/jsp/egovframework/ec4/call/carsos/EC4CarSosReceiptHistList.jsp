...
$(document).ready(function(){
    ...
    $("#searchRcptDt").daterangepicker({ //참조: http://www.daterangepicker.com/
    ...
    $('#searchRcptDt').on('apply.daterangepicker', function(ev, picker) { 
    	//console.log(picker.startDate.format('YYYY-MM-DD')); 
    	//console.log(picker.endDate.format('YYYY-MM-DD')); 
    	$("#searchBeginDe").val(picker.startDate.format('YYYY-MM-DD'));
    	$("#searchEndDe").val(picker.endDate.format('YYYY-MM-DD'));
    }); 
    $('#searchRcptDt').on('cancel.daterangepicker', function(ev, picker) {
    	//취소눌러도 검색기본조건으로 사용되도록 초기화하지 않는다
    	//$('#searchRcptDt').val(''); 
    }); 

    
    //$("#searchCarSosReceiptMngVO input[name=searchCnsltUserId]").val("${searchCarSosReceiptMngVO.searchCnsltUserId}");
    //$("#searchCarSosReceiptMngVO input[name=searchCnsltUserNm]").val("${searchCarSosReceiptMngVO.searchCnsltUserNm}");
    //$("#searchCarSosReceiptMngVO input[name=cnsltntPop]").hide();
    //$("#searchCarSosReceiptMngVO input[name=searchCnsltUserNm]").attr('readonly', true);
    ...
    // 대상고객 테이블
    listCarSosReceiptTable = $('#listCarSosReceiptTable').DataTable({
        "language": lang_kor,
        "select": {
            "style": "single"
        },
        ...
        "searching": false,
        "bServerSide": true,
        "sAjaxSource": "<c:url value='/ec4/call/carsos/selectCarSosReceiptHistList.do'/>",
        "fnServerParams": function(aoData){
            aoData.push(
            		{"name": "searchCstmrCd",   "value": $("#searchCarSosReceiptMngVO input[name=searchCstmrCd]").val()},
            		{"name": "searchCstmrNm",   "value": $("#searchCarSosReceiptMngVO input[name=searchCstmrNm]").val()},
            		{"name": "searchCnsltUserId",   "value": $("#searchCarSosReceiptMngVO input[name=searchCnsltUserId]").val()},
            		{"name": "searchCnsltUserNm",   "value": $("#searchCarSosReceiptMngVO input[name=searchCnsltUserNm]").val()},
            		{"name": "scriptKnd",   "value": $("#searchCarSosReceiptMngVO select[name=scriptKnd] option:selected").val()},
            		{"name": "scriptCd",       "value": $("#searchCarSosReceiptMngVO select[name=scriptCd] option:selected").val()},
            		{"name": "searchBeginDe",   "value": $("#searchCarSosReceiptMngVO input[name=searchBeginDe]").val()},
            		{"name": "searchEndDe",     "value": $("#searchCarSosReceiptMngVO input[name=searchEndDe]").val()},
            		{"name": "searchHpno",       "value": $("#searchCarSosReceiptMngVO input[name=searchHpno]").val()},
            		{"name": "callState",       "value": $("#searchCarSosReceiptMngVO select[name=callState] option:selected").val()},
            		{"name": "callAsign",       "value": $("#searchCarSosReceiptMngVO select[name=callAsign] option:selected").val()},
            		{"name": "searchCnsltTyCn",   "value": $("#searchCarSosReceiptMngVO input[name=cnsltTyCn]").val()},
            		{"name": "instAsignKnd",   "value": $("#searchCarSosReceiptMngVO select[name=instAsignKnd] option:selected").val()}
            );
        },
        "sServerMethod": "POST",
        "columnDefs": [
        ...
        "columns": [
            ...
            {"data":"emergencyRcptNo", "defaultContent": ""},//접수번호
            {"data":"vhcleNo", "defaultContent": ""},//차량번호
            {"data":"vin", "defaultContent": ""},//차대번호
            {"data":"cstmrPlc", "defaultContent": ""},//고객위치
            {"data":"emergencyDt", "defaultContent": ""},//출발일자
            {"data":"arrivalDt", "defaultContent": ""},//도착일자
            {"data":"endDt", "defaultContent": ""},//끝일자
            {"data":"etcMemo", "defaultContent": ""},//기타메모
            {"data":"reqCstmrNm", "defaultContent": ""}//의뢰인명
    ...
    // 검색 input search
    $("#searchCarSosReceiptMngVO input").on("keyup change", function(key){

        if(key.keyCode == 13 || $(this).hasClass("datepicker")){

        	listCarSosReceiptTable.search(this.value).draw();
        }
    ...
    // 검색 select search
    $("#searchCarSosReceiptMngVO select").on("change", function(key){
    ...
    // 선택
    listCarSosReceiptTable.on("select", function (e, dt, type, indexes) {
    ...
    $("#btnInsert").click(function(){
        ...
        var url = "<c:url value='/ec4/call/carsos/selectCarSosReceiptRegPop.do'/>";//EC4CarSosReceiptMngRegPop

        popupParams.width = "1200";
        popupParams.height = "900";
        popupParams.title = "popupCarSosReceiptMngReg";
    ...
    $("#btnUpdate").click(function(){
        ...
        var rowData = listCarSosReceiptTable.rows(".selected").data();
        if(rowData[0]){
            params.emergencyRcptNo = rowData[0].emergencyRcptNo;
            params.cstmrCd = rowData[0].cstmrCd;
            params.rcptUserId = rowData[0].rcptUserId;
            params.cnsltIncallNo = rowData[0].cnsltIncallNo;
        }
        
        // 수정 팝업 오픈
        fn_carSosReceipt_mod_popup(params);
        ...
        var callbackClass = "callback";
        var selectStyle = "single";
        var url = "<c:url value='/ec4/call/carsos/selectCarSosReceiptModPop.do'/>";//EC4CarSosReceiptMngModPop

        popupParams.width = "1200";
        popupParams.height = "900";
        popupParams.title = "popupCarSosReceiptMngMod";
        popupParams.form = document.popupForm;
    ...
    $("#searchCstmrNm").keyup(function(){
		if($("#searchCarSosReceiptMngVO input[name=searchCstmrNm]").val() == ""){
			$("#searchCarSosReceiptMngVO input[name=searchCstmrCd]").val("");
		}
	});
    ...
    /* $('#listCarSosReceiptTable tbody').on('dblclick', 'tr', function () {
    	var popupParams = {};
    	
    	var idx = $(this).index();
    	var rowData = listCarSosReceiptTable.rows(idx).data();
        ...
        var callbackClass = "callback";
        var selectStyle = "single";
        var url = "<c:url value='/ec4/call/outbd/carSosReceiptMngRegPop.do'/>";//EC4CarSosReceiptMngRegPop

        popupParams.width = "1300";
        popupParams.height = "900";
        popupParams.title = "popupCarSosReceiptMngReg";
        popupParams.form = document.searchCarSosReceiptMngVO;
...
function fn_cnsltLicense_reg_popup(params){
	...
	$("div.modal-content").load("<c:url value='/ec4/call/outbd/carSosReceiptMngRegPop.do'/>", params, function(response, status, xhr) {
	
	   if(status == "success"){
	
	       $("div.modal").modal({"backdrop": "static", "keyboard": false, "width":"1200px", "height":"800px"});
...
//수정 팝업 오픈
function fn_carSosReceipt_mod_popup(params){

	if(params.emergencyRcptNo){

		 $("#popupModal div.modal-content").load("<c:url value='/ec4/call/carsos/selectCarSosReceiptModPop.do'/>", params, function(response, status, xhr) {
...
<!-- Main content -->
    <section class="content">
		...
		<div class="row">
            <div class="col-md-12">
        		<form:form commandName="searchCarSosReceiptMngVO" name="searchCarSosReceiptMngVO" method="post" cssClass="form-horizontal">
                ...
                <div class="box">
                    ...
                    <form class="form-horizontal">
                            <div class="form-group">
                                <label for="searchRcptDt" class="col-sm-2 control-label">접수일자</label>
                                <div class="col-sm-4">
					                <div class="input-group">
					                  <div class="input-group-addon">
					                    <i class="fa fa-calendar"></i>
					                  </div>
	                                  <input type="text" class="form-control pull-right" name="searchRcptDt" id="searchRcptDt">
	                                </div>
                                </div>
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-2">
                                    <input type="text" name="searchVhcleNo" class="form-control" id="searchVhcleNo"/>
                                </div>
                                <label for="searchVin" class="col-sm-2 control-label">차대번호</label>
                                <div class="col-sm-2">
                                    <input type="text" name="searchVin" class="form-control" id="searchVin"/>
                                </div>
        	...
        	<div class="col-xs-12"> <!-- DIV 가로영역을 나누는 용도 -->
                ...
                <div class="box">
                    ...
                    <div class="box-body no-padding" style="height:750px;">
				          <table id="listCarSosReceiptTable" class="table table-hover">
				            <thead>
				              <tr>
				                <th>순번</th>
				                <th>고객코드</th>
