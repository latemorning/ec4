...
var AXSelectConverter = Class.create(AXJ, {
    ...
    bindSelect: function (objID, objSeq) {
		...
		po.push(">");
		if ($("#"+objID).prop("class").indexOf("ess_select") > -1) {
			po.push("	<div class=\"selectedText ess_select\" id=\"" + cfg.targetID + "_AX_" + objID + "_AX_SelectText\" style=\"line-height:" + (h - (borderT+borderB)) + "px;padding:0px 4px;font-size:" + fontSize + "px;\"></div>");
		}else if($("#"+objID).prop("class").indexOf("ess_select_readonly") > -1){
			po.push("	<div class=\"selectedText ess_select_readonly\" id=\"" + cfg.targetID + "_AX_" + objID + "_AX_SelectText\" style=\"line-height:" + (h - (borderT+borderB)) + "px;padding:0px 4px;font-size:" + fontSize + "px;\"></div>");
		}else{
 ...
 * @example
 ```
 axdom("Selector").prop("disabled",true);  // bindSelectDisabled(true) 18.01.03 khh변경
 ```
 */
